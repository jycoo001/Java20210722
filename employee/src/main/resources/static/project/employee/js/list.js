$(function() {
	...
	$(".tab").on("click", ".edit", function() {
		...
		/*window.open(path + "update/" + a, pa);*/
		//.find(".layui-nav-child")
		window.location.href = path + "update/" + a;
		/*window.open(path + "update/" + a,'_blank');*/
	});
	...
	$(".updateBox").click(function() {
		...
		if (check.length > 1) {
			...
			/*window.open(path + "update/" + check.text(), '_blank');*/
			window.location.href = path + "update/" + check.text();
		} else {
			layer.alert("请选择记录");
		}
	});
	...
	$(".insert").on("click", function() {
		/*window.open(path + "insert", '_blank');*/
		window.location.href = path + "insert";
	});
