...
<body>
	...
	<div class="inputview">
		...
		</span> <span class="inputspan"> <label class="inputtext">书籍:</label>
			<select class="inputselect" name="Library_id">
				<c:forEach items="${book}" var="r" varStatus="v">
					<option value="${v.index}"
						<c:if test="${(v.index+1)==info.library_id}"> selected="selected" </c:if>>${r.name}</option>
				</c:forEach>
		</select>
		</span>
	</div>
