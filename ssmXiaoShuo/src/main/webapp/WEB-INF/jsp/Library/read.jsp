					...
					<c:if test="${list.id>1}">
						<button>
							<a style="color: blue;" class="page"
								href="lastzhang?id=${list.id}">上一章</a>
						</button>
					</c:if>
					<button>
						<a style="color: blue;" class="page" href="readerindex?pageno=1">返回文章导引</a>
					</button>
					<c:if test="${list.id<count}">
						<button>
							<a style="color: blue;" class="page" href="nextzhang?id=${list.id}">下一章</a>
						</button>
					</c:if>
				</h2>
				<br>
